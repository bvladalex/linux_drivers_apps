...
unsigned char uc[4];
	...
	printf("Attempting to open bus %d\n", iChannel);
	...
	printf("Attempting to set oled address on bus %d\n", iChannel);
	...
	{
		if(write(file_i2c, oled32_initbuf, sizeof(oled32_initbuf))<=0){
			perror("Could not write data to device on this bus.\n");
			return 1;
		}
	}
	else
	{
		if(write(file_i2c, oled64_initbuf, sizeof(oled64_initbuf))<=0){
			perror("Could not write data to device on this bus.\n");
			return 1;
		}
	}
...
void oledWritePic(unsigned char *pic_array, int pic_width, int pic_height){
			int i,no_pages=0;
			if (pic_height%8==0)
				no_pages=pic_height/8;
			else
				no_pages=pic_height/8+1;
			no_pages=pic_height/8;
			for (i=0;i<no_pages;i++){
				oledSetPosition(0,i);
				oledWriteDataBlock(pic_array+pic_width*i, pic_width);
			}
		}
