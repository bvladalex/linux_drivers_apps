...
static const char *HELLO_KEYS_NAME1 = "PUSH_KEY1";
static const char *HELLO_KEYS_NAME2 = "PUSH_KEY2";
...
//the increment and decrement ISRs follow:
/* First interrupt handler */
static irqreturn_t KEY_ISR1(int irq, void *data)
{
	struct mcp4131_data *pot_data = data;
	dev_info(&(pot_data->spi->dev), "interrupt PUSH_KEY1 received. key: %s\n",
			 HELLO_KEYS_NAME1);
	spi_write(pot_data->spi, short_cmd+1, 1);
	dev_info(&(pot_data->spi->dev), "pot cursor goes up");
	return IRQ_HANDLED;
}
...
{
	struct mcp4131_data *pot_data = data;
	dev_info(&(pot_data->spi->dev), "interrupt PUSH_KEY2 received. key: %s\n",
			 HELLO_KEYS_NAME2);
	spi_write(pot_data->spi, short_cmd, 1);
	dev_info(&(pot_data->spi->dev), "pot cursor goes down");
	return IRQ_HANDLED;
}
			    ...
			    int *val, int *val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		...
		*val2=53;
		mutex_unlock(&data->lock);
		printk("value for val = %d !\n", *val);
		printk("value for val2 = %d !\n", *val2);
			     ...
			     int val, int val2, long mask)
{
	...
	case IIO_CHAN_INFO_RAW:
		if (val > data->cfg->max_pos || val < 0){
			if(val==767){
				printk("Hit exception!\n");
				err = spi_write(data->spi, short_cmd, 1);
				return err;
			}
			else if(val==676){
				printk("Hit exception!\n");
				err = spi_write(data->spi, short_cmd+1, 1);
				return err;
			}
			else
				return -EINVAL;
...
static int mcp4131_probe(struct spi_device *spi)
{
	...
	device_for_each_child_node(dev, child){
		...
		const char *label_name, *trigger;

		fwnode_property_read_string(child, "label", &label_name);


		if (strcmp(label_name,"PUSH_KEY_1") == 0) {
		...
		else if (strcmp(label_name,"PUSH_KEY_2") == 0) {
